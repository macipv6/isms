...
class RegisterAuditTest extends TestCase
{
    ...
    {
        [$customer, $project, $actor] = $this->context(); $service = app(BusinessProcessService::class);
        ...
        $service->changeStatus($process, false, $actor); $service->changeStatus($process->fresh(), false, $actor);
        ...
        foreach (['Secret name', 'Other secret', 'Private', 'secret@example.test'] as $secret) $this->assertStringNotContainsString($secret, $json);
    }

    public function test_audit_failure_rolls_back_register_write(): void
    {
        [, $project, $actor] = $this->context();
        $this->app->instance(AuditLogger::class, new class extends AuditLogger { public function record(string $eventType, ?User $actor, array $context = [], ?string $organizationId = null): AuditEvent { throw new RuntimeException('audit unavailable'); } });
        $this->expectException(RuntimeException::class);
        app(BusinessProcessService::class)->create($project, ['key' => 'SALES', 'name' => 'Sales', 'active' => true], $actor);
    }

    private function context(): array { $customer = Organization::factory()->create(['organization_type' => 'customer', 'entra_tenant_id' => null]); $project = IsmsProject::factory()->for($customer)->create(); $actor = User::factory()->for(Organization::factory()->state(['organization_type' => 'internal']))->create(); return [$customer, $project, $actor]; }
}
